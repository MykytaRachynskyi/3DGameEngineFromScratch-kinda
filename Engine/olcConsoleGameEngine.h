...
blocks - but you can draw any unicode character, using any of the colourours listed below.
There may be bugs!
...
private:
	...
	void Create(int w, int h)
	{
		...
		m_colourours = new short[w*h];
		for (int i = 0; i < w*h; i++)
		{
			m_Glyphs[i] = L' ';
			m_colourours[i] = FG_BLACK;
		}
	}
...
public:
	...
	void Setcolourour(int x, int y, short c)
	{
		if (x < 0 || x >= nWidth || y < 0 || y >= nHeight)
			return;
		else
			m_colourours[y * nWidth + x] = c;
	}
	...
	short Getcolourour(int x, int y)
	{
		if (x < 0 || x >= nWidth || y < 0 || y >= nHeight)
			return FG_BLACK;
		else
			return m_colourours[y * nWidth + x];
	}
	...
	short Samplecolourour(float x, float y)
	{
		int sx = (int)(x * (float)nWidth);
		int sy = (int)(y * (float)nHeight - 1.0f);
		if (sx < 0 || sx >= nWidth || sy < 0 || sy >= nHeight)
			...
			return m_colourours[sy * nWidth + sx];
	}
	...
	bool Save(std::wstring sFile)
	{
		...
		fwrite(m_colourours, sizeof(short), nWidth * nHeight, f);
	...
	bool Load(std::wstring sFile)
	{
		...
		std::fread(m_colourours, sizeof(short), nWidth * nHeight, f);
...
public:
	...
	virtual void Draw(int x, int y, short c = 0x2588, short colour = 0x000F)
	{
		if (x >= 0 && x < m_nScreenWidth && y >= 0 && y < m_nScreenHeight)
		{
			m_bufScreen[y * m_nScreenWidth + x].Char.UnicodeChar = c;
			m_bufScreen[y * m_nScreenWidth + x].Attributes = colour;
		}
	}

	void Fill(int x1, int y1, int x2, int y2, short c = 0x2588, short colour = 0x000F)
	{
		Clip(x1, y1);
		Clip(x2, y2);
		for (int x = x1; x < x2; x++)
	...
	void DrawString(int x, int y, std::wstring c, short colour = 0x000F)
	{
		for (size_t i = 0; i < c.size(); i++)
		{
			m_bufScreen[y * m_nScreenWidth + x + i].Char.UnicodeChar = c[i];
			m_bufScreen[y * m_nScreenWidth + x + i].Attributes = colour;
		}
	}

	void DrawStringAlpha(int x, int y, std::wstring c, short colour = 0x000F)
	{
		for (size_t i = 0; i < c.size(); i++)
		{
			if (c[i] != L' ')
	...
	void DrawLine(int x1, int y1, int x2, int y2, short c = 0x2588, short colour = 0x000F)
	{
	...
	void DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, short c = 0x2588, short colour = 0x000F)
	{
		DrawLine(x1, y1, x2, y2, c, colour);
		DrawLine(x2, y2, x3, y3, c, colour);
		DrawLine(x3, y3, x1, y1, c, colour);
	}

	// https://www.avrfreaks.net/sites/default/files/triangles.c
	void FillTriangle(int x1, int y1, int x2, int y2, int x3, int y3, short c = 0x2588, short colour = 0x000F)
	{
		auto SWAP = [](int &x, int &y) { int t = x; x = y; y = t; };
		auto drawline = [&](int sx, int ex, int ny) { for (int i = sx; i <= ex; i++) Draw(i, ny, c, colour); };
	...
	void DrawCircle(int xc, int yc, int r, short c = 0x2588, short colour = 0x000F)
	{
		int x = 0;
		int y = r;
		int p = 3 - 2 * r;
		...
		{
			Draw(xc - x, yc - y, c, colour);//upper left left
			Draw(xc - y, yc - x, c, colour);//upper upper left
			Draw(xc + y, yc - x, c, colour);//upper upper right
			Draw(xc + x, yc - y, c, colour);//upper right right
			Draw(xc - x, yc + y, c, colour);//lower left left
			Draw(xc - y, yc + x, c, colour);//lower lower left
			Draw(xc + y, yc + x, c, colour);//lower lower right
			Draw(xc + x, yc + y, c, colour);//lower right right
			if (p < 0) p += 4 * x++ + 6;
			else p += 4 * (x++ - y--) + 10;
		}
	}

	void FillCircle(int xc, int yc, int r, short c = 0x2588, short colour = 0x000F)
	{
		...
		for (int i = 0; i < sprite->nWidth; i++)
		{
			for (int j = 0; j < sprite->nHeight; j++)
			{
				if (sprite->GetGlyph(i, j) != L' ')
					Draw(x + i, y + j, sprite->GetGlyph(i, j), sprite->Getcolourour(i, j));
			}
		}
	}
		...
		for (int i = 0; i < w; i++)
		{
			for (int j = 0; j < h; j++)
			{
				if (sprite->GetGlyph(i + ox, j + oy) != L' ')
					Draw(x + i, y + j, sprite->GetGlyph(i + ox, j + oy), sprite->Getcolourour(i + ox, j + oy));
			}
		}
	}

	void DrawWireFrameModel(const std::vector<std::pair<float, float>> &vecModelCoordinates, float x, float y, float r = 0.0f, float s = 1.0f, short colour = FG_WHITE, short c = PIXEL_SOLID)
	{
		...
		for (int i = 0; i < verts + 1; i++)
		{
			...
			DrawLine((int)vecTransformedCoordinates[i % verts].first, (int)vecTransformedCoordinates[i % verts].second,
				(int)vecTransformedCoordinates[j % verts].first, (int)vecTransformedCoordinates[j % verts].second, c, colour);
		}
	}
